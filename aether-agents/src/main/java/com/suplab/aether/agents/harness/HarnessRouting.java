...
import com.agentharness.interop.LegacyAgentAdapter.LegacyResult;
...
/**
 * Routes a Grid agent's proposed decision through the agent-harness {@link Harness} for gating. The agent
 * keeps its own logic and per-branch metadata; the harness applies the centralized confidence gate and sets
 * {@code autoEnforced} (a BLOCK auto-enforces only at ≥ 0.95). Every migrated agent gates through this one
 * path, so enforcement lives in exactly one place and no agent can bypass it.
 */
public final class HarnessRouting {

    // Grid agents may emit any decision; authority BLOCK keeps every emitted action within authority, and
    ...
    /** Gate a proposed grid decision through the harness, preserving the agent's output metadata. */
